...
# Simple to use ❤️
```bash
docker-compose -f versions/2.7.3/DockerCompose/airflow.yml up -D
```
[DockerCompose](https://github.com/wiseupdata/airflow/blob/main/versions/2.7.3/DockerCompose/airflow.yml)
...
- Versions info 🐍
  - <p align="left"><a href="#version-2.7.3">2.7.3</a></p>
- Utils 🛠️
...
- [Dockerfile](https://github.com/wiseupdata/airflow/blob/main/versions/2.7.3/Docker/Dockerfile)

  > This setup it's the same for the tags: 2.7.3 and 2.7.3-ubuntu-22.04
...
```bash
docker run -it --name airflow --rm --entrypoint /bin/bash wiseupdata/airflow:2.7.3 
docker run -it --name airflow --rm --entrypoint /bin/bash wiseupdata/airflow:2.7.3-ubuntu-22.04 
```
...
docker-compose -f versions/2.7.3/DockerCompose/airflow.yml up

```

## Stop
```bash
docker-compose -f versions/2.7.3/DockerCompose/airflow.yml down
```

## Run with local Docker file
```bash

docker-compose -f versions/2.7.3/DockerCompose/local-airflow.yml up
...
```bash
docker build -t airflow ./versions/2.7.3/docker-compose-custom-img/ --no-cache
```
...
docker tag airflow wiseupdata/airflow:2.7.3
docker tag airflow wiseupdata/airflow:2.7.3-ubuntu-22.04
```

- push your image to dockerhub ♨️
```bash
docker push wiseupdata/airflow
docker push wiseupdata/airflow:2.7.3
docker push wiseupdata/airflow:2.7.3-ubuntu-22.04
```
...
sudo rm -Rf versions/2.7.3/DockerCompose/postgres
mkdir -p versions/2.7.3/DockerCompose/postgres/data
mkdir -p versions/2.7.3/DockerCompose/postgres/init

cat <<EOF >> versions/2.7.3/DockerCompose/postgres/init/init.sql
ALTER USER airflow WITH PASSWORD 'airflow';
ALTER DATABASE airflow OWNER TO airflow;
EOF

sudo rm -Rf versions/2.7.3/DockerCompose/pgadmin
mkdir -p versions/2.7.3/DockerCompose/pgadmin

sudo  rm -Rf versions/2.7.3/DockerCompose/logs
mkdir -p versions/2.7.3/DockerCompose/logs

sudo chmod 777 -R versions/2.7.3/DockerCompose
```
...
-v $PWD/versions/2.7.3/DockerCompose/dags:/opt/airflow/dags \
-v $PWD/versions/2.7.3/DockerCompose/dags:/opt/airflow/logs \
wiseupdata/airflow:2.7.3 -c "echo logged; bash"
```
...
1. [Airflow](https://airflow.apache.org/docs/apache-airflow/2.7.3/installation/installing-from-pypi.html)
